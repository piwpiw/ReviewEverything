...
- **잡 스케줄링 API**: `GET /api/cron?runNow=true (runNow=true, limit=6)`, `GET /api/cron`
...
2. **Execute**: `BackgroundJob` 레코드 PENDING 발급. `GET /api/cron?runNow=true (runNow=true, limit=6)` 병렬로 워커 가동.
