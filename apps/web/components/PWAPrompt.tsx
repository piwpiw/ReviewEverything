...
import { X, Smartphone, Share, PlusSquare, LayoutGrid } from "lucide-react";
...
export default function PWAPrompt() {
  ...
  return (
    <AnimatePresence>
      {show && (
        <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} className="fixed bottom-6 left-6 right-6 z-[200] max-w-md mx-auto">
          <div className="bg-slate-900 text-white rounded-[2.5rem] p-8 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] border border-white/10 relative overflow-hidden group">
            ...
            <button onClick={dismiss} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-white transition-colors">
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex gap-6 items-center">
              ...
              <div className="flex flex-col gap-1">
                <h3 className="text-lg font-black tracking-tight leading-none mb-1">홈 화면에 추가</h3>
                <p className="text-xs text-slate-400 font-bold leading-relaxed">
                  앱처럼 빠르게 실행하고, 알림도 더 편하게 받아보세요.
                </p>
              </div>
            </div>
                ...
                {platform === "ios" ? (
                  <>
                    <div className="flex flex-col items-center gap-1 shrink-0">
                      ...
                      <span className="text-[10px] font-black text-slate-500">공유</span>
                    </div>
                    <div className="w-px h-8 bg-white/5" />
                    <div className="flex flex-col items-center gap-1 shrink-0">
                      <PlusSquare className="w-5 h-5 text-blue-400" />
                      <span className="text-[10px] font-black text-slate-500">추가</span>
                    </div>
                    <p className="text-[11px] font-bold text-slate-300">[공유] → [홈 화면에 추가]</p>
                  </>
                ) : (
                  <>
                    <Smartphone className="w-6 h-6 text-blue-400 shrink-0" />
                    <p className="text-[11px] font-bold text-slate-300">
                      브라우저 메뉴에서 <span className="text-white font-black">[홈 화면에 추가]</span>를 선택하세요.
                    </p>
                  </>
                )}
              </div>
