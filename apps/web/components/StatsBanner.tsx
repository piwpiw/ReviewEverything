...
export default async function StatsBanner() {
    ...
    try {
        ...
        console.warn("StatsBanner: DB connection failed, using high-fidelity mock stats.");
        ...
        typeBreakdown = [
            { campaign_type: "VST", _count: { campaign_type: 8400 } } as any,
            { campaign_type: "SHP", _count: { campaign_type: 7442 } } as any,
        ];
    }
    ...
    return (
        <div className="mt-10 flex flex-wrap gap-4 items-center justify-center md:justify-start">
            ...
            {/* Type breakdown */}
            <div className="flex items-center gap-3 bg-white/70 dark:bg-slate-900/50 backdrop-blur-md px-5 py-4 rounded-2xl border border-white dark:border-slate-800 shadow-sm transition-colors">
                <div>
                    <div className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">방문형</div>
                    <div className="text-xl font-black text-slate-900 dark:text-white">{vstCount.toLocaleString()}</div>
                </div>
                <div className="w-px h-8 bg-slate-200 dark:bg-slate-800" />
                <div>
                    <div className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">배송형</div>
                    <div className="text-xl font-black text-slate-900 dark:text-white">{shpCount.toLocaleString()}</div>
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
                {platformBreakdown.slice(0, 7).map(p => (
                    <div key={p.name} className="flex items-center gap-1.5 bg-white/70 dark:bg-slate-900/50 backdrop-blur-md px-3 py-2 rounded-xl border border-white dark:border-slate-800 shadow-sm transition-colors">
                        <div className="w-2 h-2 rounded-full" style={{ background: PLATFORM_COLORS[p.name] ?? "#64748b" }} />
                        <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400">{p.name}</span>
                        <span className="text-[10px] font-black text-slate-900 dark:text-white">{p.count}</span>
                    </div>
                ))}
            </div>
        </div>
