...
const renderStatusLabel = (status?: string) => {
  if (status === "SUCCESS") return "수집 완료";
...
const resolveStatusClass = (status?: string) => {
  ...
  if (status === "RUNNING") return "bg-amber-100 text-amber-700 animate-pulse";
...
export default function ScraperStatusTable({ initialRuns }: { initialRuns: RunRow[] }) {
  if (initialRuns.length === 0) {
    return <div className="text-center p-12 text-slate-500 font-medium">수집 이력이 없습니다.</div>;
  }
      ...
      <table className="w-full text-left border-collapse whitespace-nowrap min-w-max">
        <thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500 border-b border-slate-200">
          <tr>
            ...
            <th className="p-4 font-bold">추가 수</th>
        ...
        <tbody className="text-sm font-medium text-slate-600">
          {initialRuns.map((run, idx) => (
            ...
            >
              ...
              <td className="p-4 font-black text-slate-800">{run.platform?.name || "미지정"}</td>
              <td className="p-4 text-center">
                <span
                  className={`inline-flex items-center justify-center min-w-20 px-2.5 py-1.5 rounded-lg text-xs font-black ${resolveStatusClass(
                    run.status,
                  )}`}
                >
