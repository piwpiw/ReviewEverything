...
const AUTOLOGIN_POLICY = [
  "?먮룞濡쒓렇???꾨낫??is_active 諛?auto_signin_enabled??ON??泥댄뿕?⑤쭔 ??곸쑝濡??⑸땲??",
  "梨꾨꼸 ?뺣젹 ?곗꽑?쒖쐞??auto_login + ?곌껐?곹깭(connected > needs_reauth > disconnected > error), provider ?곗꽑?쒖쐞 ?쒖엯?덈떎.",
  "?ㅻ쪟 肄붾뱶??REAUTH_REQUIRED, LOGIN_ERROR, NOT_CONNECTED, CONNECTED濡?怨좎젙 ?몄텧?⑸땲??",
];

const FAILURE_CODE_LABELS: Record<string, string> = {
  REAUTH_REQUIRED: "?ъ씤利??꾩슂",
  LOGIN_ERROR: "濡쒓렇???ㅻ쪟",
  NOT_CONNECTED: "?곌껐 ?놁쓬",
  CONNECTED: "?뺤긽 ?곌껐",
};
...
const SORT_OPTIONS = [
  { value: "auto_signin", label: "?먮룞濡쒓렇???곗꽑" },
  { value: "updated_desc", label: "理쒓렐 ?섏젙?? },
  { value: "updated_asc", label: "?ㅻ옒???? },
  { value: "name_asc", label: "?대쫫 ?ㅻ쫫李⑥닚" },
  { value: "name_desc", label: "?대쫫 ?대┝李⑥닚" },
];
...
export default function ReviewerManager() {
  ...
  const loadCreators = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(payload.error || `議고쉶 ?ㅽ뙣 (${res.status})`);
      }
      const data = (await res.json()) as CreatorsResponse;
      setItems(Array.isArray(data.items) ? data.items : []);
      setSelected(new Set());
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : "泥댄뿕??紐⑸줉??遺덈윭?ㅼ? 紐삵뻽?듬땲??";
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  const statusText = useMemo(() => {
    ...
    return `珥?${items.length}媛? ?쒖꽦 ${active}媛? ?먮룞濡쒓렇??以鍮?誘몃떖 ${failed}媛?;
  }, [items]);
    ...
    return credentials.map((credential) => {
      ...
      return (
        <span
          ...
          title={`채널: ${credential.account_label || "-"} | 프로필: ${credential.profile_name || "-"} | 실패코드: ${failureCode}`}
        >
  ...
  const patchCreator = async (id: number, payload: Record<string, unknown>) => {
    ...
    if (!res.ok) {
      ...
      throw new Error(body.error || `?섏젙 ?ㅽ뙣 (${res.status})`);
    }
  };
  ...
  const handleCreate = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        const body = (await res.json()) as { error?: string };
        throw new Error(body.error || `?깅줉 ?ㅽ뙣 (${res.status})`);
      }
      resetCreateForm();
      setStatusMessage("泥댄뿕?⑥쓣 異붽??덉뒿?덈떎.");
      await loadCreators();
    } catch (e: unknown) {
      setStatusMessage(e instanceof Error ? e.message : "泥댄뿕??異붽? ?ㅽ뙣");
    } finally {
  ...
  const handleEditSave = async (id: number, e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      resetEdit();
      setStatusMessage("泥댄뿕???뺣낫瑜??섏젙?덉뒿?덈떎.");
      await loadCreators();
    } catch (e: unknown) {
      setStatusMessage(e instanceof Error ? e.message : "?섏젙 ?ㅽ뙣");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDelete = async (item: CreatorItem) => {
    if (!window.confirm(`"${item.display_name}"瑜???젣?좉퉴??`)) return;
    setIsSubmitting(true);
    setStatusMessage(null);
    try {
      ...
      if (!res.ok) {
        const body = (await res.json()) as { error?: string };
        throw new Error(body.error || `??젣 ?ㅽ뙣 (${res.status})`);
      }
      await loadCreators();
      setStatusMessage("泥댄뿕?⑥쓣 ??젣?덉뒿?덈떎.");
    } catch (e: unknown) {
      setStatusMessage(e instanceof Error ? e.message : "??젣 ?ㅽ뙣");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleBulkAuto = async (nextAuto: boolean) => {
    ...
    try {
      ...
      setSelected(new Set());
      setStatusMessage(`?좏깮 ${selectedCount}媛??먮룞濡쒓렇???숈옉??${nextAuto ? "ON" : "OFF"}濡??곸슜?덉뒿?덈떎.`);
      await loadCreators();
    } catch (e: unknown) {
      setStatusMessage(e instanceof Error ? e.message : "?쇨큵 ?곸슜 ?ㅽ뙣");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleToggleActive = async (item: CreatorItem) => {
    ...
    try {
      ...
      await loadCreators();
      setStatusMessage(`"${item.display_name}" ?곹깭瑜?蹂寃쏀뻽?듬땲??`);
    } catch (e: unknown) {
      setStatusMessage(e instanceof Error ? e.message : "?곹깭 蹂寃??ㅽ뙣");
    }
  };
  ...
  const handleToggleAuto = async (item: CreatorItem) => {
    ...
    try {
      ...
      await loadCreators();
      setStatusMessage(`"${item.display_name}" ?먮룞濡쒓렇???좉???蹂寃쏀뻽?듬땲??`);
    } catch (e: unknown) {
      setStatusMessage(e instanceof Error ? e.message : "?좉? 蹂寃??ㅽ뙣");
    }
  };
  ...
  return (
    <div className="space-y-5">
      <section className="grid md:grid-cols-3 gap-3">
        <p className="text-[11px] text-slate-400 col-span-3">자동로그인 정책: {AUTOLOGIN_POLICY[0]}</p>
        <p className="text-[11px] text-slate-400 col-span-3">정렬 기준: {AUTOLOGIN_POLICY[1]}</p>
        <p className="text-[11px] text-slate-400 col-span-3">오류 라벨: {AUTOLOGIN_POLICY[2]}</p>
      </section>
      ...
      <section className="grid md:grid-cols-3 gap-3">
        ...
        >
          <option value="all">?꾩껜 provider</option>
          {PROVIDERS.map((item) => (
      ...
      <section className="grid md:grid-cols-3 gap-3">
        ...
        >
          {AUTO_FILTERS.map((item) => (
            <option value={item} key={item}>
              {item === "all" ? "?먮룞濡쒓렇???꾩껜" : item === "true" ? "?먮룞濡쒓렇??ON" : "?먮룞濡쒓렇??OFF"}
            </option>
          ))}
        </select>
        <select
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
        <div className="flex justify-between gap-2 items-center mb-3">
          <h3 className="text-sm font-black text-slate-200">泥댄뿕??紐⑸줉</h3>
        ...
        {loading ? <div className="text-slate-400 text-sm">濡쒕뵫 以?..</div> : null}
        {error ? (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left border-b border-slate-800">
                ...
                <th className="py-2 pr-3">泥댄뿕?⑤챸</th>
                <th className="py-2 pr-3">?곹깭</th>
                <th className="py-2 pr-3">?먮룞濡쒓렇??/th>
                <th className="py-2 pr-3">?곌껐 ?곹깭</th>
                <th className="py-2 pr-3">留덉?留?寃利?/th>
                <th className="py-2 pr-3">留덉?留??ㅽ뙣</th>
                <th className="py-2 pr-3">?≪뀡</th>
              </tr>
            </thead>
            <tbody>
              {items.length === 0 ? (
